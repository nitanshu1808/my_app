...
module Interval
  class PingFrequency
    ...
    def record_ping(user_id, time)
      raise 'Invalid arguments' if user_id.nil? || time.nil?
    ...
    def get_user_pings_per_interval(user_id, freq, start_time, end_time)
      user_ping_frequency = user_pings[user_id]
      return [] if user_ping_frequency.nil?

      intervals = prepare_partitions(freq, start_time, end_time)
      user_ping_frequency.each do |frequency|
        intervals.each do |range, val|
          if range.to_a.include? frequency
            intervals[range] += 1
            break
          end
        end
    ...
    def prepare_partitions(freq, start_time, end_time)
...
# "minute", 0, 150
# Expected outcome = [(0..59), (60..119), (120..150)]

# "hour", 10, 4000
# Expected outcome = [10-3609, 3610-4000]

# c        start_time     temp      end_time     range                  new_start_time      

# True        0             59         150     {(0..59) => 0}                 60
...
# def main
#   obj = Interval::PingFrequency.new
#   obj.record_ping("user_1", 5)
#   obj.record_ping("user_2", 15)
...
#   pp obj.user_pings
#   pp obj.get_user_pings_per_interval("user_1", "minute", 0, 150)
#   pp obj.get_user_pings_per_interval("user_3", "hour", 10, 4000)
#   pp obj.get_user_pings_per_interval("user_2", "hour", 0, 4000)
# end
